...
public class MergeConflictSequenceTest
{
  ...
  void testAdjustLineEndingsFromUnixToMac()
  {
    ...
    String testString = "Hello there\nHow are you?\n";
    String adjustLineEndings = MergeConflictSequence._adjustLineEndings(testString, mergeData);
    Assertions.assertEquals("Hello there\rHow are you?\r", adjustLineEndings);
  }
  ...
  void testAdjustLineEndingsFromMixedToMac()
  {
    ...
    String testString = "Hello there\r\nHow are you?\rThere's another one\n";
    String adjustLineEndings = MergeConflictSequence._adjustLineEndings(testString, mergeData);
    Assertions.assertEquals("Hello there\rHow are you?\rThere's another one\r", adjustLineEndings);
  }
  ...
  private static IMergeData _createMergeData(ELineEnding pLineEndingTheirs, ELineEnding pLineEndingYours)
  {
    ...
    Mockito.when(fileContentInfoTheirs.getLineEnding()).thenReturn(() -> pLineEndingTheirs);
    IFileDiff fileDiffTheirs = Mockito.mock(IFileDiff.class);
    Mockito.when(fileDiffTheirs.getFileContentInfo(EChangeSide.NEW)).thenReturn(fileContentInfoTheirs);
    IFileContentInfo fileContentInfoYours = Mockito.mock(IFileContentInfo.class);
    Mockito.when(fileContentInfoYours.getLineEnding()).thenReturn(() -> pLineEndingYours);
    IFileDiff fileDiffYours = Mockito.mock(IFileDiff.class);
    Mockito.when(fileDiffYours.getFileContentInfo(EChangeSide.NEW)).thenReturn(fileContentInfoYours);
    IMergeData mergeData = Mockito.mock(IMergeData.class);
    Mockito.when(mergeData.getDiff(EConflictSide.THEIRS)).thenReturn(fileDiffTheirs);
    Mockito.when(mergeData.getDiff(EConflictSide.YOURS)).thenReturn(fileDiffYours);
    return mergeData;
  }
}
