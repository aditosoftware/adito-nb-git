...
public class SynchronizedBoundedRangeModel extends DefaultBoundedRangeModel implements IDiscardable
{
  ...
  private boolean setValueWithoutFeedback(int n, BoundedRangeModel pSetter)
  {
    ...
    {
      SwingUtil.invokeInEDT(() -> super.setValue(n));
      return true;
    }
  }
  ...
  private class _CoupledScrollbarInfo implements IDiscardable
  {
                          ...
                          @NonNull Observable<Optional<List<IChangeDelta>>> pFileChangesEventObs, boolean pUseInverseMap)
    {
      ...
      propertyChangeDisposable = Observable.create(new PropertyChangeObservable<>(pToCouple, "model"))
          .startWithItem(Optional.of(pToCouple.getModel()))
