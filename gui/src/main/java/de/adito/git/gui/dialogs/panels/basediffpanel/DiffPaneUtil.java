...
public interface DiffPaneUtil
{
   ...
   * @param pModel      DiffPanelModel that contains the list of IFileChangeChunks describing the contents of pEditorPane and methods to get the end/start
   *                    lines of a specific chunk
   */
  ...
  static void moveCaretToPreviousChunk(JEditorPane pEditorPane, DiffPanelModel pModel)
  {
    ...
    {
      if (changeChunks.get(index).getChangeType() != EChangeType.SAME)
      {
        if (pModel.getGetEndLine().apply(changeChunks.get(index)) <= caretLine)
        {
