...
class StashChangesQuestionDialog extends RevertFilesDialog
{
  ...
  {
    super(pActionProvider, pIconLoader, pPrefStore, pFileSystemUtil, pFilesToRevert, pProjectDir);
    if (pFilesToRevert.size() == 1)
    {
      descriptionLabel.setText("<html>The following file was changed: " + pFilesToRevert.get(0).getFile().getAbsolutePath() + ". <br><br>The changes to the file have to" +
                                   " either be stashed or discarded before Git can continue</html>");
    }
    else
    {
