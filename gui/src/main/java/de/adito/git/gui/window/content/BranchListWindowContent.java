...
class BranchListWindowContent extends JPanel
{
  ...
  private void _initGui()
  {
    ...
    Observable<List<IBranch>> localSelectionObservable = Observable
        .combineLatest(localSelectionModel.selectedRows(), branchObservable, (pSelected, pBranches) -> {
    ...
    Observable<List<IBranch>> remoteSelectionObservable = Observable
        .combineLatest(remoteSelectionModel.selectedRows(), branchObservable, (pSelected, pBranches) -> {
          List<IBranch> branches = pBranches.orElse(Collections.emptyList());
          if (pSelected == null)
            return Collections.emptyList();
          return Stream.of(pSelected)
  ...
  private class _PopupStarter extends MouseAdapter
  {
    ...
    public void mouseReleased(MouseEvent pE)
    {
      if (pE.isPopupTrigger())
      {
        ...
        int row = table.rowAtPoint(pE.getPoint());
        if (row >= 0)
        {
          ...
          popupMenu.show(table, pE.getX(), pE.getY());
        }
        else
        {
