...
public class CommitHistoryTreeListItem
{
   ...
   * @param pAfterNext ICommit following pNext in the history
  ...
  public CommitHistoryTreeListItem nextItem(@NotNull ICommit pNext, @Nullable ICommit pAfterNext)
  {
    ...
    for (AncestryLine oldAncestryLine : ancestryLines)
    {
      ...
      if (oldAncestryLine.getLineType() != AncestryLine.LineType.STILLBORN)
      {
        ...
        // the only lines that are advanced are those whose parent is the current commit (lines do not fork/split/end spontaneously,
        // only after encountering their parent commit
   ...
   */
  private void _processChildren(boolean pProcessedChildren, AncestryLine pOldAncestryLine, List<AncestryLine> pUpdatedAncestryLines,
                                ICommit pNext, Map<Integer, AncestryLine> pCheckForStillBorns, int pCounter)
  {
      ...
      for (AncestryLine childLine : pOldAncestryLine.getChildLines())
      {
        if (childLine.getLineType() == AncestryLine.LineType.STILLBORN)
        {
          pUpdatedAncestryLines.add(new AncestryLine(childLine.getParent(),
                                                     childLine.getColor(), AncestryLine.LineType.STILLBORN,
                                                     childLine.getStillBornMeetingIndex(), colorRoulette));
        }
        else
        {
          AncestryLine newAncestryLine = new AncestryLine(childLine.getParent(), childLine.getColor(), colorRoulette);
          // check again for parent matching the next commit here, since if the parent being the current commit can hide the next commit
          // being the parent of the child line
  ...
  private void _resolveLines()
  {
    ...
    // numClosing: number of lines that end in this line/commit (basically number of commits that have this commit as parent - 1 || 0.
    // Used by the lower part of most lines
    ...
    for (AncestryLine currentAncestryLine : ancestryLines)
    {
      if (currentAncestryLine.getParent().equals(commit))
      {
        ...
        else
        {
          if (currentAncestryLine.getLineType() == AncestryLine.LineType.STILLBORN)
          {
            linesToDraw.add(_getCoordinatesForIndices(currentAncestryLine.getStillBornMeetingIndex(), knotIndex, true,
                                                      currentAncestryLine.getColor()));
            numStillBorn++;
          }
          else
          {
            numClosing++;
            // draw line from top of the cell (at the incoming point of the line) to the dot/knot on the line that this particular commit is on
            linesToDraw.add(_getCoordinatesForIndices((double) index - numStillBorn, knotIndex, true, currentAncestryLine.getColor()));
          }
        }
      }
      else
      {
        ...
        linesToDraw.add(_getCoordinatesForIndices((double) index - numStillBorn, (double) index - numStillBorn,
                                                  true, currentAncestryLine.getColor()));
        // draw line from the middle of the cell to the bottom of the cell, x value is determined by the position that this line will be in in the
        // next line
        ...
        linesToDraw.add(_getCoordinatesForIndices((double) index - numStillBorn,
                                                  (double) index + (numOfChildren > 0 ? numOfChildren - 1 : 0) - numClosing - numStillBorn,
                                                  false, currentAncestryLine.getColor()));
      }
      index++;
    }
   ...
   *
   * @param pKnotIndex           Index of the line that has the knot displaying the current commit
   * @param pNumOfChildren       number of children so far
   ...
   */
  private int _doChildLines(int pKnotIndex, int pNumOfChildren, AncestryLine pCurrentAncestryLine)
  {
    for (AncestryLine childLine : pCurrentAncestryLine.getChildLines())
    {
      ...
      if (childLine.getLineType() == AncestryLine.LineType.STILLBORN)
      {
        linesToDraw.add(_getCoordinatesForIndices(pKnotIndex, childLine.getStillBornMeetingIndex(), false, childLine.getColor()));
      }
      else
      {
        // draw line from the dot to the bottom of the cell, x value on the bottom is determined by the previous lines and the number of
        // children before this one
        linesToDraw.add(_getCoordinatesForIndices(pKnotIndex, (double) pKnotIndex + pNumOfChildren, false, childLine.getColor()));
        pNumOfChildren++;
      }
    }
    return pNumOfChildren;
  }
